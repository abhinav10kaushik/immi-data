...
def load_pax_in_out():
    # load the csv file
    url = "https://data.gov.au/dataset/cc5d888f-5850-47f3-815d-08289b22f5a8/resource/38bdc971-cb22-4894-b19a-814afc4e8164/download/mon_pax_web.csv"
    print("downloading 'mon_pax_web.csv' from '{}'...".format(url))
    df = pd.read_csv(url)

    # columns we want to keep
    columns = ["AIRPORT", "Year", "Month", "Int_Pax_In", "Int_Pax_Out"]
    df = df[columns]
    # keep only the entries for the year Jan 2010 – Dec 2017
    df = df.loc[(df["Year"] >= 2010) & (df["Year"] <= 2017)]
    # exclude the total passenger numbers from the data
...
def load_filtered_pax_in_out():
    ...
    for airport in all_airports:
        # if the airport name exists in the list then keep the same name
...
def load_us_exchange_rate():
    ...
    print("downloading 'f11-data.csv' from '{}'...".format(url))
    df = pd.read_csv(url)
    s = io.StringIO()
    df.to_csv(s)
...
def load_movements_csv(name):
    if name == 340101:
        url = "http://abs.gov.au/AUSSTATS/ABS@Archive.nsf/log?openagent&340101.xls&3401.0&Time%20Series%20Spreadsheet&23F247356B1A250CCA25830800119CF6&0&Jul%202018&17.09.2018&Latest"
    else:
        url = "http://abs.gov.au/AUSSTATS/ABS@Archive.nsf/log?openagent&340102.xls&3401.0&Time%20Series%20Spreadsheet&0A2A23A6256D8AB0CA25830800119F0C&0&Jul%202018&17.09.2018&Latest"

    print("downloading '{}.xls' file from '{}'...".format(name, url))
    df = pd.read_excel(url, sheet_name=1, header=1)
    s = io.StringIO()
    df.to_csv(s)
    ...
    # create a dataframe from the lists
    df = pd.DataFrame.from_records(data, columns=columns)
    # keep only the entries between year 2013 and 2017
    df = df.loc[(df["Year"] >= 2010) & (df["Year"] <= 2017)]
...
'''
Task 4 and 5 have the same functionality with the difference
that they refer to different files
'''


def load_incoming_movements():
...
def load_average_weekly_earnings():
    url = 'http://www.abs.gov.au/ausstats/meisubs.NSF/log?openagent&6302003.xls&6302.0&Time%20Series%20Spreadsheet&F35E044B5A542FDBCA2582EA00193CA8&0&May%202018&16.08.2018&Latest'
    print("downloading '6302003.xls' file...")
    df = pd.read_excel(url, sheet_name=1, header=1)
    s = io.StringIO()
    df.to_csv(s)
